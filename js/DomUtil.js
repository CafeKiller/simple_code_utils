...
/**
 * 使用对象的格式,为页面补充样式, 可以用于处理因无法修改行内样式而导致的问题
 * @param {object} rules CSS样式规则对象
 * @example
 * const cssRules = {
 *     ".font1":{
 *         "font-size":"18px",
...
/**
 * 在页面中添加按钮来对页面进行测试, 需要配合 addStylesheetRules 函数一起使用
 * @param {Array} buttonList 按钮对应的配置
 * @param {options} options.tagDOM 添加到指定DOM内,可选,默认在body内
 * @param {options} options.cssRules 添加到指定CSS规则,可选
 * @example 
 * const buttonList = [
 *     { name: '功能测试1', callback: "alert('hello world')" },
 *     { name: '功能测试2', callback: "console.log('Data.now', Data.now())" },
 *     { name: '功能测试3', callback: "console.error('error!!!')" },
 * ];
